...
void Screen::Draw(const Line2D& line, const Color& color)
{
	...
	if (moptrWindow)
	{
		...
		dx = x1 - x0; // rise and run of the line
		dy = y1 - y0; // rise and run of the line

		signed const char ix((dx > 0) - (dx < 0));  // evaluate to 1 or -1 depending on direction
		signed const char iy((dy > 0) - (dy < 0));

		dx = abs(dx) * 2; // To remove any float we times by 2 to man an int
		dy = abs(dy) * 2;
